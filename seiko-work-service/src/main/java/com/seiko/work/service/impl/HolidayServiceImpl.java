...
/**
 * 节假日 Service 实现
 * 数据源：https://holiday.ailcc.com/api/holiday/allyear/{year}（type>=2：节日、调休放假、补班）
 * 第三方 API 不支持浏览器跨域调用，故由服务端代理；结果按年缓存到 Redis（7 天）
 */
@Slf4j
@Service
public class HolidayServiceImpl implements HolidayService {
    ...
    private List<HolidayVO> fetch(int year) {
        try {
            return fetchFromAilcc(year);
        } catch (Exception e) {
            log.error("节假日数据获取失败，year={}", year, e);
        }
        throw new BusinessException(ResultCode.ERROR.getCode(), "节假日数据获取失败");
    }
     ...
     */
    private List<HolidayVO> fetchFromAilcc(int year) {
        JsonNode root = restClient.get()
                .uri("https://holiday.ailcc.com/api/holiday/allyear/{year}", year)
                .retrieve()
                .body(JsonNode.class);
        if (root == null || root.path("code").asInt(-1) != 0) {
            throw new IllegalStateException("ailcc error response");
        }
        JsonNode data = root.path("data");
        if (!data.isArray() || data.isEmpty()) {
            throw new IllegalStateException("ailcc empty response");
        }
